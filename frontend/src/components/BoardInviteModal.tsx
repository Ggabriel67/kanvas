...
import type { BoardInvitationRequest } from '../types/invitations';
import { createBoardInvitation } from '../api/invitations';
...
const BoardInviteModal: React.FC<BoardInviteModalProps> = ({ isOpen, onClose, boardId }) => {
	if (!isOpen) {
    return null;
  }
  ...
  const { user: currentUser } = useAuth();
    ...
    let request: BoardInvitationRequest = {
    ...
    try {
      await createBoardInvitation(request);
		...
		<div className="fixed inset-0 z-50 flex items-center justify-center">
			...
			<div className="relative bg-[#1e1e1e] text-gray-200 rounded-xl shadow-xl w-full max-w-lg p-6 z-10 pb-8">
				<div className="flex items-center justify-between mb-5">
					<h2 className="text-xl font-semibold">Invite user to board</h2>
					<IoMdClose 
